...
export function FeaturedBlogCard({ post }: { post: BlogPost }) {
  return (
    <article className="group relative flex flex-col  bg-white rounded-3xl border border-slate-100 p-2 sm:p-5 shadow-sm hover:shadow-md transition-all duration-300 items-center ">

      {/* --- Image Area --- */}
      <Link
        href={`/blog/${post.slug}`}
        className="block w-full overflow-hidden rounded-2xl bg-slate-50"
      >        {post.image ? (
        <Image
          src={post.image}
          alt={post.title}
